  ...
  module Converter
    ...
    class Latex < Base
      ...
      def initialize(root, options)
        ...
        @options[:latex_headers] = %w{mychapter mysection mysubsection mysubsubsection paragraph subparagraph}
      ...
      def convert_ul(el, opts)
        if !@data[:has_toc] && (el.options[:ial][:refs].include?('toc') rescue nil)
          ...
          if el.type == :ul
            latex_environment('itemize', el, inner(el, opts))
          else
            el.options[:env_opts] = '[1.]'
            latex_environment('enumerate', el, inner(el, opts))
          end
        end
      end
      ...
      def convert_table(el, opts)
        ...
        id = el.attr['id']
        title = el.attr['title']
        "\\begin{mytable}{#{id}}{#{title}}\n\\begin{longtable}{|#{align}|}#{attrs}\n\\hline\n#{inner(el, opts)}\\hline\n\\end{longtable}\n\\end{mytable}#{attrs}\n\n"
      end
      ...
      def latex_environment(type, el, text)
        attrs = attribute_list(el)
        "\\begin{#{type}}#{el.options[:env_opts]}#{latex_link_target(el)}#{attrs}\n#{text.rstrip}\n\\end{#{type}}#{attrs}\n"
      end
